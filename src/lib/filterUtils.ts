...
export function filterTransactions(
  ...
  return transactions.filter((transaction) => {
    ...
    if (andFilters.length > 0) {
      const allAndFiltersMatch = andFilters.every((filter) => {
        return evaluateFilter(transaction, filter);
      });

      if (!allAndFiltersMatch) {
        return false;
      }
    }
    ...
    const anyOrFilterMatches = orFilters.some((filter) => {
      return evaluateFilter(transaction, filter);
    });

    // Return true if any OR filter matched, false otherwise
    return anyOrFilterMatches;
  });
}
